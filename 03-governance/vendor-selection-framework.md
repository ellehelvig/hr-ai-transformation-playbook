...
**A note on the notebooks in this playbook:** the [attrition risk model](../05-notebooks/attrition-risk-modeling.ipynb) and [HR Q&A agent](../05-notebooks/hr-qa-agent-demo.ipynb) are synthetic evaluation examples, not validated systems or recommendations to buy or build these products. Define the task and evidence required first; keep individual attrition predictions internal to validation and business inquiry at segment level.
