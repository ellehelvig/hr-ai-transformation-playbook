...
- **Two Connecticut Public Act 26-15 claim records**: the 46a-60 discrimination-defense change effective 1 October 2026, and the Section 8(c) developer duty-assumption option. Registry now holds 10 records. Both are deliberately unquoted, because Connecticut publishes the act as PDF only and `verify_claim_sources.py` reads the raw body, so a quote would fail the scheduled check rather than anchor the claim. Each record says so and names the two ways to close it. Four further PA 26-15 claims moved into `BACKLOG.md` rather than being registered on guessed sources.
